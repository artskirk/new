...
/**
 * This command opens a port in the firewall.
 *
 * @author Philipp Heckel <ph@datto.com>
 */
class FirewallOpenCommand extends Command
{
    protected static $defaultName = 'firewall:open';
    ...
    {
        $this
            ->setDescription('Opens a port in the firewalld zone.')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($isPermanent) {
            $this->firewallService->open((int)$port, $zone, $protocol, true);
            $this->firewalldUserOverrideManager->addOverridePort($zone, "$port/$protocol");
        } else {
            $this->firewallService->open((int)$port, $zone, $protocol, false);
        }

        $output->writeln("Added port: " . $port .
            " to zone: " . $zone . " protocol: " . $protocol .
